...
ir_data <- ir %>% dplyr::select(1:8, # ID and weight
                        ...
                        v024,v025, # place
                        v104,v105,v175, # migration status
...
round(svytable(~teen_preg + birth_gp + v025, design = dhs, na.action = na.pass)/1e6,0)
...
round(svytable(~teen_preg + loss_gp + v025, design = dhs, na.action = na.pass)/1e6,0)
...
Remove outliers, log transformation and taking out not significant variables: NO DIFFERENCE

```{r modelfit}
# check 10 outliers *** only after generating model ***
plot(modelm_5, which = 4, id.n = 20) 
# remove outliers
fitdata <- finaldata [- c(70,178,445,509,721,769,904,1216,1650,
1686,1721,1783,1787,1823,1839,1856,2008,2065,2066),]
#dim(fitdata)

# create design using fitdata
dhsfit <- svydesign(id = ~v021, strata = ~v023, weights = ~v005, data = fitdata)

# develop model by (1) using log transformed quantitative (2) remove < 0.05 variables from model 1 - 4
modelfit <- svyglm(teen_preg ~ factor(v025) + 
                     log(v012) + factor (edu_gp) + factor (marital) + 
                     # factor (v190) + factor (employ_current) +
                     factor (contra_know_any) + factor (fp_info) +
                     # factor (v467d) + factor (v467c) +
                     factor (v467b) + factor (v467f) +
                     # factor (hv219) + 
                     log(hv012),
...
## summarizing, can check colinearlity together
summ(modelfit, vifs = TRUE) # lowest AIC so far AIC = 1500.12 
summ(modelm_5, vifs = TRUE) # AIC 1692.89
...
Alternative dataset - using only married: NO DIFFERENCE

```{r alternative}
# remove married
marrydata <- finaldata %>% filter (marital != 1)
dim(marrydata)
...
# testing summ package for summary and forest plot
## summarizing, can check colinearlity together
summ(modelmarry, vifs = TRUE)

## plotting
plot_summs(modelmarry) # only one model
```

Using stepAIC as Tutor suggested

```{r stepAIC}
# apply stepwise regression (forward) for full model (modelm_5)
stepfull <- stepAIC(modelm_5, direction = "forward")
stepfull$anova # no improvement at AIC 1721.479 (different from summ function)

# apply stepwise regression (forward) for fitted model (modelfit)
stepfit <- stepAIC(modelfit, direction = "forward")
stepfit$anova # no improvement at AIC 1519.622 (different from summ function)
```
