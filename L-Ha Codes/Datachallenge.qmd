...
glm_gest <- svyglm(stillbirth~v025*gest, OR_design, family=quasibinomial())
sb_forest[9,] <-
  c("preterm","0.011",
    exp(coef(glm_gest))[[2]],
...
glm_bmi <- svyglm(stillbirth~v025*bmi, OR_design, family=quasibinomial())
sb_forest[10,] <-
  c("bmi","<0.001",
    exp(coef(glm_bmi))[[2]],
...
psb <- # create a special df to assess previous stillbirths
  NR[ # include only if:
    NR$caseid %in% lb_id | # the woman has experienced a live birth, OR
      NR$caseid %in% sb_id, # the woman has experienced a stillbirth
  ]
...
ggplot(df, aes(y = Factor, x = OR, xmin = CI_low, xmax = CI_high)) +
  ...
  coord_flip()  # Flip the axes to display the factors on the y-axis
